<template>
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>
                Đợt xét tốt nghiệp / Đợt thi {{ dotthi.dt_ten }}
            </h1>
            <ol class="breadcrumb">
        ...
        <section class="content">
            <a href='dot-xet-tot-nghiep' class="btn btn-default" style="margin-bottom:6px;">
                ...
                <div class="col-md-12 col-sm-12">
                    <div class="box box-widget">
                        <!-- <a href="{{ route('export-ds-dat-diem-thi-tot-nghiep', ['lh_id'=> $lopHoc->lh_id, 'dt_id'=> $dotThi->dt_id]) }}"
                            ...
                            <i class="fa fa-file-excel-o"></i> Xuất kết quả danh sách thi đạt tốt nghiệp
                        </a> -->
                            ...
                            <table class="table table-striped table-bordered no-margin" style="margin-top:12px !important">
                                <thead>
                                    <tr>
                                        ...
                                        <!-- <th>Quyết định</th> -->
                                        <th class="w-100-p text-center">Kết quả thi tốt nghiệp</th>
                                ...
                                <tbody>
                                    <tr v-for="(lh, index) in table.list.data" :key="lh.lh_id">
                                        ...
                                        <td class="text-left">
                                            <a :href="'/dot-xet-tot-nghiep/ket-qua-hoc-tap/' + lh.lh_id + '/' + dt_id"
                                                title="Danh sách dự xét" class="btn bg-purple btn-sm"><i class="fa fa-eye"></i> Danh sách kết quả thi
                                            </a>
                                        </td>
                                        <!-- <td>
...
import Label from '../../../../vendor/laravel/breeze/stubs/inertia-vue/resources/js/Components/Label.vue';
    ...
    const consumer = {
        getListLopHoc: function (dt_id) {
            const url = 'http://localhost/tthl/public/apilop-hoc/dot-thi-lop-hoc/'+dt_id+'/'+ window.location.search;
        ...
        saveOrUpdate: function (formData) {
            if (formData.lh_id == null) {
                return axios.post('http://localhost/tthl/public/apilop-hoc', formData);
            } else {
                return axios.put('http://localhost/tthl/public/apilop-hoc/' + formData.lh_id, formData);
            }
        },
        destroy: function (lh_id) {
            return axios.delete('http://localhost/tthl/public/apilop-hoc/' + lh_id);
        },
        getListKhoaDaoTao: function () {
            const url = 'http://localhost/tthl/public/apikhoa-dao-tao/all';
            return axios.get(url).then(response => response.data);
        },
        getListQuyetDinh: function () {
            const url = 'http://localhost/tthl/public/apiquyet-dinh/all/0';
            return axios.get(url).then(response => response.data);
        },
        getListNienKhoa: function () {
            const url = 'http://localhost/tthl/public/apinien-khoa/all';
            return axios.get(url).then(response => response.data);
        },
        getListHeDaoTao: function () {
            const url = 'http://localhost/tthl/public/apihe-dao-tao/all';
            return axios.get(url).then(response => response.data);
        },
    }
