...
import Label from '../../../../vendor/laravel/breeze/stubs/inertia-vue/resources/js/Components/Label.vue';
    ...
    const consumer = {
        getListLopHoc: function (dt_id) {
            const url = 'http://localhost/cea-2.1/public/api/lop-hoc/dot-thi-lop-hoc/'+dt_id+'/'+ window.location.search;
        ...
        saveOrUpdate: function (formData) {
            if (formData.lh_id == null) {
                return axios.post('http://localhost/cea-2.1/public/api/lop-hoc', formData);
            } else {
                return axios.put('http://localhost/cea-2.1/public/api/lop-hoc/' + formData.lh_id, formData);
            }
        },
        destroy: function (lh_id) {
            return axios.delete('http://localhost/cea-2.1/public/api/lop-hoc/' + lh_id);
        },
        getListKhoaDaoTao: function () {
            const url = 'http://localhost/cea-2.1/public/api/khoa-dao-tao/all';
            return axios.get(url).then(response => response.data);
        },
