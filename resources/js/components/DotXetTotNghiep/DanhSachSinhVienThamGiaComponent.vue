...
import Label from '../../../../vendor/laravel/breeze/stubs/inertia-vue/resources/js/Components/Label.vue';
    ...
    const consumer = {
        getListSinhVien: function (dxtn_id) {
            const url = 'http://localhost/tthl11/public/api/dot-xet-tot-nghiep/'+dxtn_id+'/danhsachsinhvien'+ window.location.search;
            return axios.get(url).then(response => response.data);
        },
        capNhatSinhVienDotThi: function (dt_id,formData) {
            return axios.post('http://localhost/tthl11/public/api/dot-thi/'+dt_id+'/danhsachsinhvien', formData);
        },
        capNhatTrangThaiSinhVien: function (dxtn_id, dt_id, lh_id, sv_id, svd_ghichu, svxtn_vipham) {
            return axios.get('http://localhost/tthl11/public/api/dot-xet-tot-nghiep/'+dxtn_id+'/cap-nhat-trang-thai-sv-dxtn?dt_id='+ dt_id +'&lhid='+lh_id+'&svid='+sv_id+'&svxtn_ghichu=' + svd_ghichu + '&svxtn_vipham=' + svxtn_vipham );
        },
        // xoaSinhVienThamGia: function (dt_id, lh_id, sv_id) {
        //     return axios.get('http://localhost/tthl11/public/api/dot-thi/'+dt_id+'/xoa-sinh-vien-tham-gia?lhid='+lh_id+'&svid='+sv_id);
        // },
