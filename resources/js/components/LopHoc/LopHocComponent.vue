              ...
              <table class="table table-striped table-bordered no-margin">
                <thead>
                  <tr>
                    ...
                    <th class="w-13">Chương trình đào tạo</th>
                    <th>Quyết định</th>
                    <th>Quy chế</th>
                ...
                <tbody>
                  <tr v-for="(lh, index) in table.list.data" :key="lh.lh_id">
                    ...
                    <td>
                      <span v-if="lh.quyet_dinh" :title="lh.quyet_dinh.qd_ten"
                        >{{ lh.quyet_dinh.qd_ma }},
                        {{ lh.quyet_dinh.qd_ngay | moment }}</span
                      >
                    </td>
                    <td>
                      <span v-if="lh.lh_nienche == 0">Quy chế 2020</span>
                      <span v-if="lh.lh_nienche == 1">Quy chế 2022</span>
                    </td>
                    <td>
                      <a
                        :href="lh.edit_url"
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">
              {{
                editForm.reference == Object ? "Thêm Lớp học" : editForm.reference.lh_ten
              }}
            </h4>
          </div>
          <div class="modal-body">
            ...
            <form-group :errors="editForm.errors" :field="'lh_ma'">
              <label>Mã lớp</label>
              <input type="text" class="form-control" v-model="editForm.model.lh_ma" />
            </form-group>
            <form-group :errors="editForm.errors" :field="'lh_ten'">
              <label>Tên lớp</label>
              <input type="text" class="form-control" v-model="editForm.model.lh_ten" />
            </form-group>
            <form-group :errors="editForm.errors" :field="'lh_nienche'">
              <label>Quy chế</label>
              <select class="form-control" v-model="editForm.model.lh_nienche">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="callout callout-info">
              ...
              <p>
                Chỉ có thể sao chép lớp học cùng hệ đào tạo và cùng ngành
                nghề đào tạo
              </p>
            </div>
          </div>
...
const consumer = {
  getListLopHoc: function () {
    const url = "http://localhost/tthl/public/api/lop-hoc/paginate" + window.location.search;
  ...
  saveOrUpdate: function (formData) {
    if (formData.lh_id == null) {
      return axios.post("http://localhost/tthl/public/api/lop-hoc", formData);
    } else {
      return axios.put("http://localhost/tthl/public/api/lop-hoc/" + formData.lh_id, formData);
    }
  },
  destroy: function (lh_id) {
    return axios.delete("http://localhost/tthl/public/api/lop-hoc/" + lh_id);
  },
  getListKhoaDaoTao: function (hdt_id) {
    const url = "http://localhost/tthl/public/api/khoa-dao-tao/all?hedaotao=" + hdt_id;
    return axios.get(url).then((response) => response.data);
  },
  getListQuyetDinh: function () {
    const url = "http://localhost/tthl/public/api/quyet-dinh/all/0";
    return axios.get(url).then((response) => response.data);
  },
  ...
  duplicate: function (formData) {
    return axios.post("http://localhost/tthl/public/api/lop-hoc/duplicate/" + formData.lh_id, formData);
  }
};
            ...
            .filter((item) => {
              return (
                (item.he_dao_tao.hdt_ten.toLowerCase() == "cao đẳng" &&
                  this.quyenCaoDang) ||
                (item.he_dao_tao.hdt_ten.toLowerCase() == "trung cấp" &&
                  this.quyenTrungCap)
              );
            })
            .map((item) => {
