                ...
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group">
                      <label>Mã hoặc tên ngành, nghề</label>
                      <input
                        type="text"
                        name="search"
          ...
          <div class="box box-widget">
            <div class="box-body">
              <table class="table table-striped table-bordered no-margin">
                ...
                <tbody>
                  <tr v-for="(nn, index) in table.list.data" :key="nn.nn_id">
                    ...
                    <td class="text-center">
                      ...
                      <button
                        ...
                        title="Xóa"
                        v-if="!nn.khoa_dao_tao_exists && !nn.mon_hoc_exists"
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <form-group :errors="editForm.errors" :field="'nn_ma'">
              <label>Mã</label>
              <input type="text" class="form-control" v-model="editForm.model.nn_ma" />
            </form-group>
            <form-group :errors="editForm.errors" :field="'nn_ten'">
              <label>Ngành, nghề</label>
              <input type="text" class="form-control" v-model="editForm.model.nn_ten" />
            </form-group>
          </div>
...
const consumer = {
  getListNganhNghe: function () {
    const url = "http://localhost/cea_3.0/public/api/nganh-nghe" + window.location.search;/** Phong API */
    return axios.get(url).then((response) => response.data);
  },
  getNganhNghe: function (nn_id) {
    const url = "http://localhost/cea_3.0/public/api/nganh-nghe/" + nn_id;/** Phong API */
    return axios.get(url).then((response) => response.data);
  },
  getListHeDaoTao: function () {
    const url = "http://localhost/cea_3.0/public/api/he-dao-tao/all";/** Phong API */
    return axios.get(url).then((response) => response.data);
  },
  saveOrUpdate: function (formData) {
    if (formData.nn_id == null) {
      return axios.post("http://localhost/cea_3.0/public/api/nganh-nghe", formData);/** Phong API */
    } else {
      return axios.put("http://localhost/cea_3.0/public/api/nganh-nghe/" + formData.nn_id, formData);/** Phong API */
    }
  },
  destroy: function (nn_id) {
    return axios.delete("http://localhost/cea_3.0/public/api/nganh-nghe/" + nn_id); /** Phong API */
  },
};
  ...
  methods: {
    loadDanhSachHeDaoTao: function () {
      consumer.getListHeDaoTao().then((data) => {
        ...
        console.log(this.listHeDaoTao.select2);
      });
    },
    reloadList: function () {
      var vm = this;
