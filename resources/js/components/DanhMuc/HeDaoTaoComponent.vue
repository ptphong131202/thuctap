...
<script>
    ...
    const consumer = {
        getListHeDaoTao: function () {
            const url = 'http://localhost/tthl11/public/api/he-dao-tao' + window.location.search;
            return axios.get(url)
                    .then(response => response.data);
        },
        getHeDaotao: function (hdt_id) {
            const url = 'http://localhost/tthl11/public/api/he-dao-tao/' + hdt_id;
            return axios.get(url)
                    .then(response => response.data);
        },
        saveOrUpdate: function (formData) {
            if (formData.hdt_id == null) {
                return axios.post('http://localhost/tthl11/public/api/he-dao-tao', formData);
            } else {
                return axios.put('http://localhost/tthl11/public/api/he-dao-tao/' + formData.hdt_id, formData);
            }
        },
        destroy: function (hdt_id) {
            return axios.delete('http://localhost/tthl11/public/api/he-dao-tao/' + hdt_id);
        }
    }
