...
<script>
    const consumer = {
        getUser: function (user_id) {
            const url = 'http://localhost/cea_4.0/public/apiuser/' + user_id;
            return axios.get(url)
                    .then(response => response.data);
        },
        saveOrUpdate: function (formData) {
            if (formData.user_id == null) {
                return axios.post('http://localhost/cea_4.0/public/apiuser', formData);
            } else {
                return axios.put('http://localhost/cea_4.0/public/apiuser/' + formData.user_id, formData);
            }
        },
